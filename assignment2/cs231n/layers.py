...
def affine_backward(dout, cache):
    ...
    dw = np.matmul(np.transpose(y),dout)
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(N):
        for j in range(F):
            for k in range(int(Hd)):
                for l in range(int(Wd)):
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(0, H - pool_height + 1, stride):
        for j in range(0, W - pool_width + 1, stride):
            patch = x[:, :, i:i+pool_height, j:j + pool_width]
            patch = patch.reshape(N, C, pool_height * pool_width)
            patch = np.max(patch, axis = 2)
            height = int(i/stride)
            width = int(j/stride)
            out[:,:,height,width] = patch
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(0, H - pool_height + 1, stride):
                for j in range(0, W - pool_width + 1, stride):
                    patch = x[n, c, i:i+pool_height, j:j + pool_width].reshape(pool_height * pool_width)
                    arg_max = np.argmax(patch)
                    gradient = np.zeros(patch.size)
                    height = i // stride
                    width = j // stride
                    gradient[arg_max] = dout[n,c,height,width]
                    gradient = gradient.reshape(pool_height, pool_width)
                    dx[n,c,i:i+pool_height,j:j+pool_width] += gradient
